...
void FuncState<EventType>::onEnter(IFSM<EventType>& fsm, const EventType& event, const w4::variant::Variant& params)
{
    if (onEnterFunc)
    {
...
template<typename EventType>
void FuncState<EventType>::onLeave(IFSM<EventType>& fsm, const EventType& event, const w4::variant::Variant& params)
{
    if (onLeaveFunc)
    {
...
template<typename... FsmTransition>
bool FSM<FsmTransition...>::processEvent(const EventType& event, const variant::Variant& params)
{
    if (!m_inited)
    {
        return false;
...
template<typename... FsmTransition>
void FSM<FsmTransition...>::onEnter(IFSM<EventType>& fsm, const EventType& event, const variant::Variant& params)
{
    m_currentState = m_startState;
    (m_states.*m_onEnterCallbacks[m_currentState])(*this, event, params);
}

template<typename... FsmTransition> [[maybe_unused]] void FSM<FsmTransition...>::onLeave(IFSM<EventType>& fsm, const EventType& event, const variant::Variant& params)
{
...
template<typename... FsmTransition>
template<typename State>
void FSM<FsmTransition...>::init(const EventType& event, const variant::Variant& params)
{
